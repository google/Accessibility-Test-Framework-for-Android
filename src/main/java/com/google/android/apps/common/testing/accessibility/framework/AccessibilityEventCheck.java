...
public abstract class AccessibilityEventCheck extends AccessibilityCheck {
   ...
   * state relevant to the previous evaluation, if needed. It is guaranteed that {@link
   * #shouldHandleEvent(AccessibilityEvent)} or {@link #runCheckOnEvent(AccessibilityEvent)} will
   * not be invoked after execution of this method has begun until a new logical test run is
   ...
   *     any. If no issues are found, or if an issue cannot be identified from the stream of {@link
   *     AccessibilityEvent}s observed, an empty collection will be returned.
   */
